...
class GroupMembership(DiffSyncModel):
    ...
    A membership is a relationship between a user (identified by email) and a group.
    """
    ...
    def create(cls, adapter, ids, attrs):
        ...
        if hasattr(adapter, 'pending_operations'):
            adapter.pending_operations.append(('create', membership.user_email, membership.group_name))
    ...
    def delete(self) -> Optional["GroupMembership"]:
        ...
        if hasattr(self.adapter, 'pending_operations'):
            self.adapter.pending_operations.append(('delete', self.user_email, self.group_name))
